...
public class Apriori {
	...
	static List<List<String>> transactionList = new ArrayList<List< String>>(); //dataset
	static List<Rule> ruleList = new ArrayList<Rule>(); // kurallar
	static List<Set<String>> largeItemList = new ArrayList<Set<String>>(); // ekrana basılcak itemler
	
	public static void main(String[] args) {

		double minSupport = 0.75, minConfidence = 0.1; 
		String inputCsv = "C:\\Users\\fuatk\\Desktop\\RealLifeDataSet.csv";
		
		File inputFile = new File(inputCsv); // file içine atıyoruz
		List<Set<Set<String>>> largeItemSetList = new ArrayList<Set<Set<String>>>(); 
		
		//Getting individual Items and their counts
		Set<String> itemSet = new HashSet<String>(); 
		Map<String,Integer> itemSupportMap = new HashMap<String,Integer>(); //supportlarla beraber tutulacak  
		Map<Set<String>,Integer> largeItemSupportMap = new HashMap<Set<String>,Integer>(); // supportlarla beraber tutulacak
		ValueComparator bvc =  new ValueComparator(largeItemSupportMap); //büyükten küçüğe sıralama
		TreeMap<Set<String>, Integer> sortedSupportMap = new TreeMap<Set<String>, Integer>(bvc); // sıralanmış liste  
		try
		{
			...
			Iterator<Entry<String, Integer>> iterator = itemSupportMap.entrySet().iterator(); //her birinden kaç tane geçiyor bak
			...
			while(iterator.hasNext())
			{
				...
				if((double)(value)/transactionList.size() >= minSupport) // min sup oranından büyükse temp e ata
				{
					temp.add(key);
				}
			}
			...
			setString.add(temp);
			largeItemSetList.add(setString); // veri tipleri karışıyor diye böyle bir yöntem yaptık.
			
			int k = 1; 
			while(largeItemSetList.get(k-1).size() != 0) //dolayısıyla while içinde olucak
			{
				Set<Set<String>> addToLargeItemSetList = new HashSet<Set<String>>(); // eklenicek minsupu geçen itemler
				ArrayList<String> entryList = new ArrayList<String>(176); //girilen liste 
				ArrayList<Set<String>> preCkList = new ArrayList<Set<String>>(176); //önceki liste
				ArrayList<Set<String>> ckList = new ArrayList<Set<String>>(176); // yeni liste

				Iterator<Set<String>> iterator3 = largeItemSetList.get(k-1).iterator(); //say bakalım 
				while(iterator3.hasNext()) //iterator bitene kadar
				{
					Set<String> s = iterator3.next(); //elemanı snin içine at
					for(String string : s) 
					{
						entryList.add(string); // her elemanı entryliste ekle
					}
				}
				
				preCkList = getSubsets(entryList); //getsubsets çağır onu preckliste ata
				...
				{
					//bi öncekinde var mı yok mu yoksa sil
